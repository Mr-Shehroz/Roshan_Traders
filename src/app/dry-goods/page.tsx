...
export default function DryGoodsPage() {
  ...
  async function load() {
    setLoading(true);
    const res = await fetch("/api/products?type=dryGood");
        ...
        <main className="max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h1 className="font-serif text-2xl sm:text-3xl" style={{ color: "var(--ink)" }}>
              {editing ? "Edit Dry Good" : "Add Dry Good"}
            </h1>
          </div>
  ...
  return (
    <div style={{ background: "var(--bg)", minHeight: "100vh" }}>
      ...
      <main className="max-w-[1440px] mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 fade-up">
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: "var(--ink3)" }}>
              {products.length} products · daal, rice, spices & more
            </p>
          </div>
        ...
        {/* Search + filter */}
        <div className="flex flex-col sm:flex-row gap-2 mb-4 fade-up fade-up-1">
        ...
        {loading && (
          <div className="flex items-center justify-center py-16">
            <div
              ...
              style={{ borderColor: "var(--border2)", borderTopColor: "var(--accent2)" }}
            />
          </div>
        )}
        ...
        {!loading && filtered.length === 0 && (
          <div className="card py-16 text-center fade-up">
            ...
            <p className="font-semibold mb-1" style={{ color: "var(--ink)" }}>
              {search || catFilter ? "No results found" : "No dry goods yet"}
            </p>
            <p className="text-sm" style={{ color: "var(--ink3)" }}>
              {search || catFilter ? "Try a different filter" : "Click \"Add Dry Good\" to get started"}
            </p>
          </div>
        )}

        {/* Products grouped by category on desktop */}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs mt-0.5" style={{ color: "var(--ink3)" }}>
                      {p.variants?.length || 0} weight variant{p.variants?.length !== 1 ? "s" : ""} · priced by kg
                    </p>
                  </div>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-serif text-xl" style={{ color: "var(--ink)" }}>Log Stock</h2>
              <button
